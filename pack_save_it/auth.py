...
from .encrypt import pad16, aes_cbc_encryptor, pad8, des_ecb_encryptor
from .exception import NotAllowedService, NeedCaptcha, InvalidCaptha, IncorrectLoginCredentials, \
    UnknownAuthserverExcepyion, NotLogined, MultiSessionConflict, ParseError
...
class _AuthPageParser(HTMLParser):
    ...
    def handle_data(self, data):
        if self._js_start and not self._js_end:
            ...
            raise UnknownAuthserverExcepyion(
...
def login_sso(session: Session,
    ...
    if login_resp.status_code == 302:
        ...
        if error_code == _SSO_CAPTCHA_ERROR_CODE:
            return InvalidCaptha()
        elif error_code is None:
            raise UnknownAuthserverExcepyion("No error code")
        else:
            raise UnknownAuthserverExcepyion(
